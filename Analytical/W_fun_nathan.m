function [W, t_vec] = W_fun_nathan( ThermalDiffus_p, ThermalConduc_p, ThermalDiffus_b,...
   ThermalConduc_b, L_polymer, L_base, t_vec, h_c)

%This function calculates the integral for W cited in the paper "Analytical
...
for i=2:length(t_vec)
    ...
    F3 = -2*L_p*cos((pi()-delta)*.5)./(t*a_p).^.5; %for E

    F4 = -2*L_p*sin(.5*(pi()-delta))./(t*a_p).^.5; %for alpha


 
    eqn = @(Z) (1 - (abs((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))).*exp(Z*F3)).^2+...
        2*abs((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))).*exp(Z*F3).*...
        sin(Z*F4+angle((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))))*1i)./...
        (1 + (abs((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))).*exp(Z*F3)).^2-...
        2*abs((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))).*exp(Z*F3).*...
        cos(Z*F4+angle((ep_1-(Z+F1)./(Z-F1).*exp(Z*F2))./...
        (1-ep_1*(Z+F1)./(Z-F1).*exp(Z*F2))))).*...
...
% W=interp1(t_vec,W_interp,t_old); 
% t_vec=t_old;
