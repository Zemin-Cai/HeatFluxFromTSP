function [q_s, t, epsilon_ba] = HFComputation_Analytical_FiniteBase( ThermalDiffus_p, ThermalConduc_p, ThermalDiffus_b,...
    ThermalConduc_b, L_polymer, L_base, Theata_ps, t, h_c)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% HFComputation_Analytical_FiniteBase: calculate the heat flux with
% finite-thickness base
% 
% Syntax:  HFComputation_Analytical_FiniteBase(...)
%
% Input arguments:
...
for k = 2:max(size(t))
    ...
    t_value = (k-1)*dt;
    syms r;
%     IntegralFuncStr = strcat(['(2*(1-((', num2str(epsilon_ba), '.*sqrt(cthe)+', num2str(epsilon_ba), '*', num2str(h_c_head), '-(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./',num2str(ThermalDiffus_b), ')))./(sqrt(cthe)+',...
%         num2str(h_c_head), '-', num2str(epsilon_ba), '.*(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))).^2).*exp(-cthe.*', num2str(t_value), '))./(sqrt(cthe).*(1+((', num2str(epsilon_ba),...
%         '.*sqrt(cthe)+', num2str(epsilon_ba), '.*', num2str(h_c_head), '-(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))./(sqrt(cthe)+', num2str(h_c_head), '-', num2str(epsilon_ba), '.*(sqrt(cthe)-',...
%         num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./' num2str(ThermalDiffus_b), ')))).^2-2*abs((', num2str(epsilon_ba), '.*sqrt(cthe)+', num2str(epsilon_ba), '.*', num2str(h_c_head), '-(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base),...
%         '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))./(sqrt(cthe)+', num2str(h_c_head), '-', num2str(epsilon_ba), '.*(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))).*cos(atan((', num2str(epsilon_ba),...
%         '.*sqrt(cthe)+', num2str(epsilon_ba), '.*', num2str(h_c_head), '-(sqrt(cthe)-', num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))./(sqrt(cthe)+', num2str(h_c_head), '-', num2str(epsilon_ba), '.*(sqrt(cthe)-',...
%         num2str(h_c_head), ').*exp(-2*', num2str(L_base), '.*sqrt(cthe./', num2str(ThermalDiffus_b), ')))))))']);
     IntegralFuncStr = ((1-((epsilon_ba.*sqrt(r)*i+epsilon_ba*h_c_head-(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))./(sqrt(r)*i+h_c_head-epsilon_ba.*(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))).^2).*exp(-r.*t_value))./(sqrt(r).*...
         (1+((epsilon_ba.*sqrt(r)*i+epsilon_ba.*h_c_head-(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))./(sqrt(r)*i+h_c_head-epsilon_ba.*(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))).^2-2*abs((epsilon_ba.*sqrt(r)*i+epsilon_ba.*...
         h_c_head-(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))./(sqrt(r)*i+h_c_head-epsilon_ba.*(sqrt(r)*i-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)*i))).*cos((-2*L_polymer.*sqrt(r./ThermalDiffus_p))+atan((epsilon_ba.*sqrt(r)+...
         epsilon_ba.*h_c_head-(sqrt(r)-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)))./(sqrt(r)+h_c_head-epsilon_ba.*(sqrt(r)-h_c_head).*exp(-2*L_base.*sqrt(r./ThermalDiffus_b)))))));
    
%     IntegralFuncStr = strcat(['(exp(-cthe.^2)./(1+', num2str(epsilon_ba), '^2-2*', num2str(epsilon_ba), '*cos(2*',...
%            num2str(L_polymer), './sqrt(', num2str(ThermalDiffus_p*t_value), ').*cthe))']);
%     IntegralFuncStr = strcat(['(exp(-' num2str(ThermalDiffus_p*t_value/4*L^2) '*cthe.^2)./(1+', num2str(epsilon_ba), '^2-2*', num2str(epsilon_ba), '*cos(cthe)))'])
    %Whead_Value(k-1) = sqrt(t_value/pi)*IntegralComputation_Infinite(IntegralFuncStr, 'cthe', torrence);
    F = int(IntegralFuncStr, r, 0, Inf);
    Whead_Value(k-1) = sqrt(t_value/pi)*eval(F);
...
for k = 2:max(size(t))
    ...
    for j = 2:k
        t_value1 = t(k) - t(j);
        if(t_value1 ~= 0)
            Whead_Value1 = Whead_Value(k-j);
        else
            Whead_Value1 = 0;
        end
        t_value2 = t(k) - t(j-1);
        if(t_value2 ~= 0)
            Whead_Value2 = Whead_Value(k-j+1);
        else
            Whead_Value2 = 0;
        end
        ...
        sum = sum + (Theata_ps(j) - Theata_ps(j-1))*(Whead_Value1 + Whead_Value2)/(sqrt(t_value1) + sqrt(t_value2));
    ...
    q_s(k) = ThermalConduc_p/sqrt(pi*ThermalDiffus_p)*sum;
...
% figure;
% plot(t, q_s, '-.g');
% title('The recovered heat flux');
% xlabel('Time');
% ylabel('Recovered heat flus');
